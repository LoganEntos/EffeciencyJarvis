...
const BUILTINS = [
  ...
  },
  {
    ...
    hint: 'This session is UI / front-end work on the dashboard. BEHAVIOR BEFORE BEAUTY: before editing anything, '
      ...
      + 'changed — NEVER touch 5757; tear the throwaway down with `powershell -File scripts/kill-port.ps1 -Port 5758` '
      + '— strictly by port, never a raw taskkill/Stop-Process by PID or "stop all node", which has killed the real '
      + 'primary hub before), not by screenshots, static HTML, or unit tests alone. Edit the real source in '
