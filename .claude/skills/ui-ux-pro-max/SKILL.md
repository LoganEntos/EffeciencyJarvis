...
## Hub adaptation (claude-hub local — read this first)

- **No Python on this machine.** Ignore `scripts/*.py` and any `--domain` CLI
  examples. Query the database directly with the Grep tool over
  `data/*.csv` and `data/stacks/*.csv` (they are plain CSV; search by keyword,
  e.g. pattern "dashboard" in `products.csv`, a palette name in `colors.csv`,
  a font name in `typography.csv`).
- **This hub is a zero-dependency vanilla JS/CSS app.** Prefer
  `data/stacks/html-tailwind.csv` conceptually but output plain CSS (no
  Tailwind build step). React/Vue/etc. stack files apply only when a run's
  task explicitly targets those frameworks.
- **Fonts are vendored locally.** Before importing from Google Fonts, check
  `claude-dashboard/vendor/css/fonts.css` — JetBrains Mono, IBM Plex Sans,
  Fraunces, Newsreader, Source Serif 4, Space Mono, DM Mono, VT323, Archivo,
  Bricolage Grotesque, Hanken Grotesk, Instrument Serif are already served at
  `/vendor/fonts/` (artifact CSP allows /vendor/ only). Map font-pairing picks
  to these families when possible.
- Pairs with the local `ui-design` skill (fast anti-slop rules); this skill is
  the deep catalog. Source: github.com/nextlevelbuilder/ui-ux-pro-max-skill
  (MIT — LICENSE.upstream.txt).
