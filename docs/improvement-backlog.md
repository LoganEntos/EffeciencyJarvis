...
> A literal `|` inside a cell MUST be escaped as `\|` (standard GFM) or the parser
> rejects the whole row (cell-count mismatch).
...
| C52 | lib/autopilot.js:60 | `parseBacklog()` silently drops any row whose issue/fix/loc cell contains a `|` (raw `||`, a `/x|y/` regex, or escaped `\|`). The regex uses `([^|]*)` per column anchored on exactly 8 pipes; a 9th pipe fails the `^…$` match and the row vanishes — never dispatched, never counted in backlogOpen. The scout prompt actively tells agents to append rows describing code, so a queued fix mentioning a pipe is invisible forever | split the row on unescaped `|` (or handle `\|`) instead of one all-columns `[^|]*` regex | M | low | ✅ 2026-07-28 |
