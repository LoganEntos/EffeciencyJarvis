...
| C44 | lib/voice.js:143 | Kokoro boot warm-start raced the dying predecessor after the supervised /api/restart: health-probe saw the old sidecar still up, skipped spawning, then the orphan died — kokoro stayed offline until a manual start | after a supervised restart, re-probe health ~10s after boot (second chance) or have startSidecar kill/adopt an orphaned sidecar on port 8791 before spawning | S | low | ⬜ |
