...
| R3 | **Auto session summaries** — Sessions no longer need the manual "summarize" click; a cheap Haiku one-shot writes a 2-3 sentence debrief for each transcript, cached on disk (`data/session-summaries.json`) so unchanged sessions never re-summarize. Background idle sweep on boot + `GET /api/session-summaries` / `POST …/build`; the tab auto-fills any missing summary on open. Hub-internal one-shots (distill/summary children) detected + labeled, model refusals dropped. `lib/sessionsum.js` | this commit |
| R4 | **Files: image thumbnails + day grouping** — core (thumbs via traversal-guarded `/api/files/view`, Today/Yesterday/weekday day-group headers) shipped `b63479c`; this pass bumps thumbnails 32→44px and makes them **click-to-enlarge** in a full-size lightbox (Esc/backdrop to close) so you can actually see the context a run was given. `assets/files.js`, `assets/style.css` | `b63479c` + this commit |
| R2 | **Navigable libraries** — Agents / Skills / Commands share one enhanced `listView`: live filter + **collapsible groups** (agents by model tier, skills/commands A–Z) with per-group counts + A→Z/Z→A sort + collapse/expand-all. No more flat dumps; one consistent UI across all three. `assets/app.js`, `assets/style.css` | this commit |
| N4 | **Schedules polish + run-tier badges** — run-history rows now show `⚡ ULTRA CODE` (max-effort), `▲ effort N/5`, and `⟡ fable5` badges so god-prompt / ultra runs are identifiable at a glance; schedule rows get a prominent `◷ next in Xm` countdown pill (obvious enabled/paused state) and a clickable last-run status chip that opens the run. `assets/runhistory.js`, `assets/tasks.js` | this commit |
| R0★ | **Overview token-burn panel (stretch)** — new "token burn · 7-day by tier" panel: per-day stacked bars split by model tier (cheap/mid/heavy) from run history, plus the burn rate (`tok/hr today`) + `this wk` total that `/api/usage` already computed but the page was discarding. Tokens + %/rates only, no dollars. `assets/overview.js`, `assets/overview.css` | this commit |
...
| # | Item | Where |
|---|------|-------|
| S-ND | **No-dollars sweep (user directive) + Jarvis operator console + Overview real-metrics + O2 skills cleanup** — every `$` removed app-wide in favor of tokens (`fmtTok`) + completion/routing % (`/api/stats/today`, token-window `/api/usage`, budget concept deleted); Jarvis tab reshaped into persona-cards + live-conversation/prompt-workspace deck (background handoff run, reviewed — fixed its undefined-`fmtEvent` crash); Overview plan-usage bars → model-distribution/success panel from real history; design-suite skills 6→2 | `39e6ed6`, `045ee51`, `573212a` |
...
| S-JV | **Jarvis tab (user request) — finalized 2026-07-16** — the hub's voice face: large canvas orb mirroring HubVoice state (idle breath / listening ripples / thinking arc / speaking pulse / dashed call halo), tap-to-talk + hold-for-call delegating to the existing voice.js path, one-tap persona chips, fold-out soul editor (`/api/personas/get` + `/save`), and a one-shot soul handoff injected on persona switch (pattern from acnlabs/OpenPersona, commit 6cd71f2). Roster completed: Jarvis + Dispatch + Sage. **Module split:** `jarvistab.js` (404 L, main tab) + `jarvisorb.js` (186 L, canvas orb) + `jarvischat.js` (130 L, live chat); `jarvis.js` (112 L) remains cross-tab helper (initJarvis, analyzePromptComplexity, jarvisDistill, DISTILL_MIN_WORDS, JARVIS_HUE, jarvisHueOf). **Defects fixed:** transcript poller resumes after chat send, chat send button re-enables on error, holding-in-context panel anchors to session id, duplicate HUE/hueOf hoisted. Smoke 88/88 green. | `assets/jarvistab.js`, `assets/jarvis.css`, `assets/jarvisorb.js`, `assets/jarvischat.js`, `assets/jarvis.js`, `lib/personas.js`, `personas/` |
